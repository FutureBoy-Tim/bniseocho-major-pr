...
### 자동 동작
- `data/members.json`의 `invitations` 배열에서 **현재 시각 기준 자동 선택**해 메인에 큰 카드로 표시
- **전환 시점**: 모임 당일 **오전 09:00** (예: 5/27 09:00 전까지는 5/27 초대장, 09:00 이후부터 다음 주차 6/3 초대장)
  - 모임 시작은 06:20이지만 모임 진행 중에도 그 초대장이 메인에 떠 있고, 모임 끝난 직후(09:00) 다음 주차로 자동 전환
- 그 외 미래 주차 초대장은 "다음 주차 초대장 미리보기"로 작게 노출 (똑같이 09:00 기준)
